...
//                        OkGoHelper.dnsOverHttps.setUrl(url.isEmpty() ? null : HttpUrl.get(url));
        ...
        findViewById(R.id.llLiveEpgSetting).setOnClickListener(new View.OnClickListener() {
            @Override public void onClick(View v) {
                FastClickCheckUtil.check(v);
                SettingDialog dialog = new SettingDialog(mActivity, false);
                dialog.show();
            }
        });
        // 独立 EPG 订阅菜单
        findViewById(R.id.llEpgSubscriptionSetting).setOnClickListener(new View.OnClickListener() {
            @Override public void onClick(View v) {
                FastClickCheckUtil.check(v);
                SettingDialog dialog = new SettingDialog(mActivity, true);
                dialog.show();
            }
        });
    }
